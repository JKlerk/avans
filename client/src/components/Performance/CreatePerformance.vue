...
export default {
    ...
    computed:{
        getArtist(){
            var specific = this.$parent.getArtists().find(element => this.performance.artist_id === element.id)
        ...
        getStage(){
            var specific = this.$parent.getStages().find(element => this.performance.stage_id === element.id)
            if(specific !== undefined){
                return specific
            } else{
                return ''
